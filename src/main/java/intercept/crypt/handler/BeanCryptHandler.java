...
public class BeanCryptHandler implements CryptHandler<Object> {
    ...
    private static final ConcurrentHashMap<Class, List<CryptFiled>> CLASS_ENCRYPT_MAP =
            new ConcurrentHashMap<>();
    private static final ConcurrentHashMap<Class, List<CryptFiled>> CLASS_DECRYPT_MAP =
            new ConcurrentHashMap<>();

    private static Object apply(Object bean) {
        try {
    ...
    public Object encrypt(Object bean, CryptField cryptField) {
        ...
        try {
            ...
            result =
                    CryptInterceptor.OLD_AND_NEW_OBJ_MAP.computeIfAbsent(
                            bean, BeanCryptHandler::apply);
        } catch (Exception e) {
            e.printStackTrace();
            throw new InterceptRuntimeException();
        }
        List<CryptFiled> filedList =
                CLASS_ENCRYPT_MAP.computeIfAbsent(result.getClass(), this::getEncryptFields);
        filedList.forEach(
                cryptFiled -> {
                    try {
                        ...
                        if (obj != null) {
                            Object encrypted =
                                    CryptHandlerFactory.getCryptHandler(obj, cryptFiled.cryptField)
    ...
    public Object decrypt(Object param, CryptField cryptField) {
        ...
        List<CryptFiled> filedList =
                CLASS_DECRYPT_MAP.computeIfAbsent(param.getClass(), this::getDecryptFields);
        filedList.forEach(
                cryptFiled -> {
                    try {
                        cryptFiled.field.setAccessible(true);
